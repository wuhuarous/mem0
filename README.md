...
在聊天应用中实际使用 mem0 后，踩了不少坑，以下是最核心的一条建议：

### 不要用 mem0 自带的记忆抽取，自己做抽取，mem0 只负责存储和检索

**问题：** mem0 内置的记忆抽取机制是**每轮对话都触发**，不管强弱记忆都会被抽取并存入。这会导致：

1. **大量低质量记忆**：单轮对话上下文不足，抽取出来的记忆碎片化严重（如"用户说好"、"用户提到了一个名字"）
2. **token 浪费**：每轮都调用 LLM 做记忆抽取，成本翻倍
3. **检索噪音**：存储了太多无关紧要的"弱记忆"，后续搜索时有用信息被淹没
...
**核心思路：**
- 将**单轮抽取**改为**多轮积累后抽取**（比如每 5-10 轮对话，或对话结束时触发一次）
- 多轮上下文提供了完整的前后语义，抽取出的记忆质量显著更高
- mem0 退化为纯粹的**记忆存储和向量检索**引擎，通过 `add()` 写入、`search()` 查询

**效果对比：**

| 方案 | 记忆质量 | Token 消耗 | 检索准确度 |
|---|---|---|---|
| mem0 内置每轮抽取 | 碎片化、大量噪音 | 高（每轮都调用 LLM） | 低（噪音干扰） |
| 业务侧多轮抽取 | 语义完整、精准 | 低（按需触发） | 高（信噪比好） |
...
for message in conversation:
    mem0.add(message, user_id="user_123")  # 每轮触发，质量差

# 推荐：业务侧积累多轮后，自己抽取，mem0 只存
conversation_history = []

for message in conversation:
    conversation_history.append(message)

    # 每隔 N 轮，或在对话结束时，用业务侧 LLM 做一次高质量记忆抽取
    if should_extract_memory(conversation_history):
        memories = extract_memories_with_llm(conversation_history)  # 自己的抽取逻辑
        for memory in memories:
            mem0.add(memory, user_id="user_123")  # mem0 只负责存储

# 检索时正常使用 mem0 search
relevant = mem0.search("用户偏好的编程语言", user_id="user_123")
```
