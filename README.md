...
在聊天应用中，mem0 的记忆抽取和存储检索可以灵活组合。以下是我们在实践中总结的一种高效架构。

### 推荐方案：业务侧多轮对话记忆抽取 + mem0 存储检索

多轮对话积累足够的上下文后再进行记忆抽取，前后语义更加完整，记忆质量显著更高。
...
**核心思路：**
- 业务侧积累多轮对话后统一抽取（比如每 5-10 轮，或对话结束时触发）
- 多轮上下文提供完整的语义场景，抽取出的记忆更加精准
- mem0 专注于**记忆存储和向量检索**，通过 `add()` 写入、`search()` 查询
...
for message in conversation:
    ...
    # 每隔 N 轮，或在对话结束时，用业务侧 LLM 做一次记忆抽取
    if should_extract_memory(conversation_history):
        memories = extract_memories_with_llm(conversation_history)
        for memory in memories:
            mem0.add(memory, user_id="user_123")
